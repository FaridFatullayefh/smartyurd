...
class Subscription(db.Model):
    ...
    seher = db.Column(db.Boolean, default=False)
    gunorta = db.Column(db.Boolean, default=False)
    axsam = db.Column(db.Boolean, default=False)
    seher_date = db.Column(db.DateTime, nullable=True)
    gunorta_date = db.Column(db.DateTime, nullable=True)
    axsam_date = db.Column(db.DateTime, nullable=True)
    
    user = db.relationship('User', backref=db.backref('subscriptions', lazy=True))

    def __repr__(self):
        return f'<Subscription {self.user_id} - {self.package}>'
...
class LoginLog(db.Model):
    ...
    def __repr__(self): 
        return f'<LoginLog {self.ip_address} at {self.timestamp.strftime("%Y-%m-%d %H:%M:%S")}>'
...
def admin_dashboard():
    if request.method == 'POST':
        ...
        if existing_subscription:
            flash(f'{email} üçün abunəlik mövcuddur !', 'danger')
            return redirect(url_for('admin_dashboard'))

        subscription = Subscription(user_id=user.id, package=package)
        db.session.add(subscription)
        db.session.commit()
        flash(f'{email} üçün abunəlik uğurla yaradıldı !', 'success')

    subscriptions = Subscription.query.all()
    return render_template('admin_dashboard.html', subscriptions=subscriptions)


@app.route('/canteen/delete_subscription/<int:subscription_id>', methods=['POST'])
@admin_required
def delete_subscription(subscription_id):
    subscription = Subscription.query.get(subscription_id)
    
    if not subscription:
        flash('Abunəlik Tapılmadı!', 'danger')
        return redirect(url_for('admin_dashboard'))

    db.session.delete(subscription)
    db.session.commit()
    
    flash('Abunəlik uğurla silindi !', 'success')
    return redirect(url_for('admin_dashboard'))



@app.route('/canteen/update_meal/<int:subscription_id>', methods=['POST'])
@admin_required
def update_meal(subscription_id):
    meal_type = request.form.get('meal_type')
    
    subscription = Subscription.query.get(subscription_id)
    
    if subscription.package == 'seher_gunorta_axsam':
        if meal_type == 'seher':
            subscription.seher = True
            subscription.seher_date = datetime.now() 
        elif meal_type == 'gunorta':
            subscription.gunorta= True
            subscription.gunorta_date = datetime.now()  
        elif meal_type == 'axsam':
            subscription.axsam = True
            subscription.axsam_date = datetime.now()  
    elif subscription.package == 'gunorta_axsam':
        if meal_type == 'gunorta':
            subscription.gunorta = True
            subscription.gunorta_date = datetime.now()  
        elif meal_type == 'axsam':
            subscription.axsam = True
            subscription.axsam_date = datetime.now()  
    elif subscription.package == 'seher_axsam':
        if meal_type == 'seher':
            subscription.seher = True
            subscription.seher_date = datetime.now()  
        elif meal_type == 'axsam':
            ...
            subscription.axsam_date = datetime.now() 
    db.session.commit()
    flash(f'Dəyişiklik uğurla edildi !', 'success')
    return redirect(url_for('admin_dashboard'))

@app.route('/canteen/reset_meals', methods=['POST'])
@admin_required
def reset_meals():
    subscriptions = Subscription.query.all()

    for subscription in subscriptions:
        subscription.seher = False
        subscription.gunorta = False
        subscription.axsam = False

    db.session.commit()
    flash('Bütün işarələr silindi !', 'success')
    return redirect(url_for('admin_dashboard'))
...
def create_admin_users():
    with app.app_context():
        ...
        canteen_user = Admin.query.filter_by(username='canteen').first()
        ...
        if not canteen_user:
            canteen_user = Admin(
                username='canteen',
                password=generate_password_hash('canteen')
            )
            db.session.add(canteen_user)
            print("Canteen user created")

        # Commit the changes to the database
        db.session.commit()
create_admin_users()

@app.route('/canteen/logout')
@admin_required  
def canteen_logout():
    session.clear()    
